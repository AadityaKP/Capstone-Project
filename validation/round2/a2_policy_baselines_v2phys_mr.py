"""A2 robustness rows under v2 physics + recoverable shocks (Addendum A
companion; no LLM). Completes the robustness panel for the most realistic
configuration.

Same design as a2_policy_baselines_v2phys.py - noop/random/heuristic/
boardroom, 50 matched seeds x 120 months, deterministic_rng, paired
Wilcoxon / Hedges-g / Holm, v2 flags with financing off (matching the
recorded v2phys panel; see a3_decomp.py docstring for the financing note) -
plus shock_recovery="mean_revert".

Writes ONLY new files: policy_comparison_v2phys_mr.csv,
policy_comparison_episodes_v2phys_mr.csv,
statistical_tests_policy_baselines_v2phys_mr.csv.
"""
...
N_SEEDS = 50
V2_MR_CONFIG = {"deterministic_rng": True, "marketing_curve": "v2",
                "competitive_entry": "scale_neutral",
                "shock_recovery": "mean_revert"}


def run_episode(policy: str, seed: int) -> dict:
    env = StartupEnv(initial_config=dict(V2_MR_CONFIG))
    env.reset(seed=seed)
    random.seed(seed)
    np.random.seed(seed)
    agent = None
    ...
    s = env.state
    return dict(physics_version="v2", shock_recovery="mean_revert",
...
def main() -> None:
    ...
    df.to_csv(OUT / "policy_comparison_episodes_v2phys_mr.csv", index=False)
    ...
    summary["shock_recovery"] = "mean_revert"
    summary.to_csv(OUT / "policy_comparison_v2phys_mr.csv", index=False)
    ...
    for a, b in combinations(["noop", "random", "heuristic", "boardroom"], 2):
        for m in metrics:
            ...
            tests.append(dict(test="A2_paired_v2phys_mr", metric=m, arm_a=a, arm_b=b,
                              n=int(mask.sum()),
    ...
    tdf.to_csv(OUT / "statistical_tests_policy_baselines_v2phys_mr.csv", index=False)
